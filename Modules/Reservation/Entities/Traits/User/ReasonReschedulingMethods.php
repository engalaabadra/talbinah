...
trait ReasonReschedulingMethods{
 //get data -paginates-//
    ...
    public function paginatesDataMethod($request, $model,$lang=null){
        ...
        if($lang && $result) return $model->where(['main_lang'=>$lang,'type'=>0])->paginate($request->total);
        else return $model->where('type',0)->paginate($request->total);
    }
    //get data -all- //
    ...
    public function allDataMethod($model,$lang=null){
        $result=array_key_exists($lang, config('languages'));
        if($lang && $result) return $model->where(['main_lang'=>$lang,'type'=>0])->get();
        else return $model->where('type',0)->get();
    }
